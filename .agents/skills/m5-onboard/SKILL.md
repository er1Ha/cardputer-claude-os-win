...
Users running `onboard.py` directly in their own terminal (not via Codex) will see all output live — no changes needed there.
...
The known apps name `buddy` resolves to the `buddy/device/` directory in this repo (custom launcher + Hello + Codex Buddy BLE client + Snake). Any other `--apps` value is treated as a filesystem path.
...
- **Codex Buddy over BLE.** First time only: in Codex Desktop, **Help → Troubleshooting → Enable Developer Tools** (one-time, persists across launches). Then **Developer menu → Hardware Buddy → Connect**. BLE works regardless of the WiFi state — the link to Codex.app is local.
...
- `pyserial` — vendored at `.Codex/skills/m5-onboard/scripts/vendor/serial/` (pinned 3.5, BSD-3-Clause).
...
Python itself has to exist before this skill can do anything — you can't bootstrap an interpreter from inside one. Same story for `git` (needed once, to clone the skill and bundle repos). Codex's responsible for detecting both and installing whatever's missing *before* running any `scripts/*.py` invocation. Detection is just running `python3 --version` / `python --version` and `git --version` — if either fails, Codex fetches them with the host's native package manager before anything else.

**Per-OS dependency bootstrap (Codex's responsibility if missing):**
